...
// DoubleTagger sends 802.1Q double-tagged ICMP probes for VLAN hopping.
type DoubleTagger struct {
	Interface string
	SourceMAC string
	SrcVLAN   int
	DstVLAN   int
...
func (d *DoubleTagger) Run() error {
	if d.SrcVLAN < 0 || d.SrcVLAN > 4095 {
		return fmt.Errorf("invalid --src-vlan %d (must be 0-4095)", d.SrcVLAN)
	}
	if d.DstVLAN < 0 || d.DstVLAN > 4095 {
		return fmt.Errorf("invalid --dst-vlan %d (must be 0-4095)", d.DstVLAN)
	}
	...
	srcMAC, err := resolveMAC(iface, d.SourceMAC)
	if err != nil {
		return err
	}
	...
	go func() {
		<-sig
		fmt.Printf("\n%s Double-tag stopped.\n", dot1qTag)
		os.Exit(0)
	}()

	frame := buildDoubleTagFrame(srcMAC, srcIP, d.SrcVLAN, d.DstVLAN, d.icmpPayload())
	fmt.Printf("%s Sent 802.1Q double-tag frame on %s from %s (outer VLAN %d, inner VLAN %d)\n",
		dot1qTag, iface.Name, srcMAC, d.SrcVLAN, d.DstVLAN)
...
func (d *DoubleTagger) icmpPayload() string {
